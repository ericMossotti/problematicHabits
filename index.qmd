...
eval_palette <- function(palette_name, n = 10, pal_type) {
  
  if (pal_type == "c") {
    return(paletteer_c(palette_name, n = n))
    } else if (pal_type == "d") {
      return(paletteer_d(palette_name, n = n))
    } else if (pal_type == "dynamic") {
      return(paletteer_dynamic(palette_name, n = n))
    }
}
...
library(harrypotter)
library(paletteer)
# paletteer_d(palette = "basetheme::deepblue")
# palette = paletteer::paletteer_c("scico::berlin", 100)
...
pal_df <- tibble(
  ...
  pals = list(
    ...
    'orange',
    'yellow',
    eval_palette("harrypotter::hufflepuff", 10, 'c'),
    eval_palette("harrypotter::ravenclaw", 10, 'c')
    )
  )
...
grp_names = c('id', 'quarter', 'weekday')

rDataFrame <- r_df |>
  dplyr::group_by(dplyr::across(dplyr::all_of(grp_names)))

r_table <- gt(r_df, groupname_col = 'id', row_group_as_column = TRUE) |>
  data_color(palette = paletteer::paletteer_d("nord::frost"),
             columns = 'relative_date_PCIAT',
             target_columns = gt::everything()
             ) |>
  gt::tab_style(
    style = list(
      cell_fill(from_column())
    ),
    locations = cells_body(columns = c('quarter', 'weekday', 'relative_date_PCIAT'))
    )|>
  tab_header(title = 'title', subtitle = 'subtitle')
...
def prepare_data(
  ...
  preprocessor = None) -> Tuple[np.ndarray, np.ndarray, ColumnTransformer]:
    ...
    cat_cols = ['basic_demos_enroll_season', 'basic_demos_sex', 
    'common_weekday', 'common_quarter']
...
def save_model(
  ...
  path: str):
  model.save_model(f"{path}/model.txt")
...
# Save model
save_model(
  final_model, preprocessor, lgb_params, "data/models")
