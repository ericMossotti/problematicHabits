...
Alongside plotting, here are some relatively simpler functions developed with the intention to append and apply consistent theming to ggplots. R libraries are currently preferred for plotting outputs as they can be extensively customized [@wickham_ggplot2_2016].
...
I noticed that using enum data types significantly sped up many database operations, as suggested in this article [@holanda_duckdb_2021]. Enums are similar to the category data type in Python, and the factor datatype in R.
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(eval_palette("pals::ocean.balance", 10, 'c', 1))
  )
...
**Hypothesis**:\
More time spent indoors on the internet might be associated with lower light exposure during daytime (Q1-Q2, Q2-Q3) and higher light exposure during nighttime (min-Q1, Q3-max).
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(
    eval_palette("harrypotter::slytherin", 20, 'c', 1))
  )
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(
    eval_palette("harrypotter::gryffindor", 20, 'c', 1))
  )
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(eval_palette("harrypotter::ravenclaw", 20, 'c', 1))
  )
...
for key, value in summary_frame.items():
  try:
    ...
    print(f"'{value[1]}' was written")
  except:
    print(f"'{value[1]}' was not written")
```
:::
...
#| code-summary: "Releases all memory that the database was using and removes the connection."
#| eval: true
