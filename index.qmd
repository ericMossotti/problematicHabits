...
I used some notable packages to give the tables and plots some styling [@hvitfeldt_paletteer_2021].

#### Tables

I developed a means to apply formatting to every column in a table using a desired paletteer palette if desired. I'm also utilizing gt tables due to the degree I can customize output [@iannone_gt_2025]. Principles stemming from functional programming, in particular, are implemented here as they seem to be most appropriate to achieve the desired effect.
...
I've developed functions to apply consistent theming to plots. Currently, I prefer R's ecosystem for outputs as they can be extensively customized [@wickham_ggplot2_2016].
...
I noticed that using Enum data types significantly sped up many database operations, as suggested in this article [@holanda_duckdb_2021].
...
Not sure why the data had hyphens in the columns names, originally. These hyphens interfere with SQL query operations, so I have the mind to replace them with underscore characters. I'm not going to overwrite the original CSV's as I wish to maintain integrity and a clear trail of data processing steps. Also, having the original data to fall back on if need be, without re-downloading could be helpful.

### Meaningful Pipeline

Doing my best to apply as many transformations as I can while I read the data into the database from the files to minimize steps needed to derive meaning from the data.
...
The BMI risk categories found at, [@zierle-ghosh_physiology_2024], have been borrowed for this project. The SQL query syntax, therefore, aims to reflect a structure of evidence-based classification. Though not used in this project, it might be useful to classify lux statistics in a way that reflect a classification standard [@actigraphcorp_lux_2024].

### Lux by Hour of Day Quantiles
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(eval_palette("harrypotter::hufflepuff", 20, 'c', 1))
  )
...
### ML Paramter Rationale

Key Parameters in lgb_params

This may or may not be optimal, but I am trying new things and showing my work as I go, so to speak. Since `sii` is a multi-class categorical variable, the task is a classification problem. Adjusting the `lgb_params`, evaluation metrics, and handling predictions ensures the model correctly interprets and solves the problem.
...
### [:A deeper explanation of model parameter selections](#lgb)
...
try:
  ...
  print(f"data/output exists")
```

SQL Optimization: Queries are pre-aggregated and load only required columns.

Memory Efficiency: Lazy loading with DuckDB ensures only necessary data is processed.

Scalability: The pipeline supports chunking or streaming for large datasets.
...
def load_and_merge_data_optimized(
    conn: duckdb.DuckDBPyConnection, 
    dataset_type: str
    ) -> pd.DataFrame:
    """
    Optimized function to load, aggregate, and merge data using DuckDB.
    """
    if dataset_type == 'train':
        ...
        WITH TrainBase AS (
            SELECT id,
                   ...
                   COALESCE(MODE(basic_demos_sex::INTEGER), 0) AS sex,
                   ...
                   COALESCE(MAX(sii::INTEGER), 0) AS sii
            FROM TrainCsv
            GROUP BY id
        ),
        TrainActivity AS (
        ...
        SELECT 
          TrainBase.*, 
          TrainActivity.light_mean, 
          TrainActivity.light_std
        FROM TrainBase
        ...
        SELECT 
          TestBase.*, 
          TestActivity.light_mean, 
          TestActivity.light_max,
          TestActivity.light_std,
          TestActivity.common_quarter
        FROM TestBase
...
def objective(trial, conn, target_col):
    ...
    params = {
        ...
        'num_class': 4,  # Ensure the number of classes matches the dataset
        'boosting_type': 'gbdt',
    ...
    for train_idx, val_idx in tss.split(X):
        ...
        # Make predictions and compute validation log loss
        val_preds = model.predict(X_val)  # This produces probabilities
        val_logloss = log_loss(y_val, val_preds)
        val_loglosses.append(val_logloss)

    # Compute summary statistics
    mean_logloss = np.mean(val_loglosses)
    std_logloss = np.std(val_loglosses)

    # Save results and summary in user_attrs
    trial.set_user_attr("cv_results", val_loglosses)
    trial.set_user_attr("cv_summary", {
        "mean_logloss": mean_logloss,
        "std_logloss": std_logloss
    })

    # Return the mean validation log loss (minimize this)
    return mean_logloss

# Hyperparameter Tuning with Optuna
def tune_hyperparameters(conn, target_col, n_trials = 50):
    """
    Tune LightGBM hyperparameters using Optuna and save CV results/summary.
    """
    # Create a study object
    study = optuna.create_study(direction="minimize")

    # Optimize using the custom objective function
    study.optimize(
        lambda trial: objective(trial, conn, target_col),
        n_trials=n_trials
    )
    ...
    # Safely extract `cv_results` and `cv_summary`
    cv_results = best_trial.user_attrs.get("cv_results", {})
    cv_summary = best_trial.user_attrs.get("cv_summary", {})

    # Create output directory if it doesn't exist
    os.makedirs("data/output", exist_ok = True)

    # Save the results to JSON files
    with open("data/output/cv_summary.json", "w") as f:
        json.dump(cv_summary, f)

    with open("data/output/cv_results.json", "w") as f:
...
def train_final_model(conn, target_col, lgb_params, save_model=True):
    """
    Train the final LightGBM model using the best hyperparameters, save it, 
    and return training/validation metrics.
    """
    # Prepare training data
    _, X_train, y_train = prepare_data(conn, 'train', target_col)
    ...
    # Compute metrics
    train_accuracy = accuracy_score(y_train_split, y_train_pred_labels)
    val_accuracy = accuracy_score(y_val_split, y_val_pred_labels)
    train_logloss = log_loss(y_train_split, y_train_pred)
    val_logloss = log_loss(y_val_split, y_val_pred)

    # Print or log the metrics
    print(f"Train Accuracy: {train_accuracy:.4f}, Train Log Loss: {train_logloss:.4f}")
    
    print(f"Validation Accuracy: {val_accuracy:.4f}, Validation Log Loss: {val_logloss:.4f}")
    ...
    if save_model:
        ...
        # Save the model
        model.save_model(os.path.join(output_dir, "final_lightgbm_model.txt"))
...
def predict_test_data(
  conn, target_col, model_file = "data/model_output/final_lightgbm_model.txt"):
    """
    Load the test data and make predictions using the trained model.
    """
    # Prepare test data
    test_df, X_test, _ = prepare_data(conn, 'test', target_col = "sii")
    ...
    # Make predictions
    # Outputs probabilities for multiclass classification
    predictions = model.predict(
      X_test, predict_disable_shape_check = True) 
    
    # Optionally return predictions along with IDs for traceability
...
if __name__ == "__main__":
    ...
    # Tune hyperparameters and save CV results/summary
    study = tune_hyperparameters(
      conn, target_col, n_trials = 50)
    ...
    final_model, metrics = train_final_model(conn, target_col, best_params)

    print("Final Model Metrics:")
    
    print(f"Train Accuracy: {metrics['train_accuracy']:.4f}, Train Log Loss: {metrics['train_logloss']:.4f}")
    
    print(f"Validation Accuracy: {metrics['val_accuracy']:.4f}, Validation Log Loss: {metrics['val_logloss']:.4f}")

    # Save the final model predictions
    test_ids, test_predictions = predict_test_data(conn, target_col)
    
    os.makedirs("data/output", exist_ok = True)
    
    predictions_df = pd.DataFrame({
        'id': test_ids,
        'predictions': test_predictions.argmax(axis=1)  # Get predicted class
    })
    
    predictions_df.to_csv("data/output/test_predictions.csv", index = False)

    print("Predictions saved to data/output/test_predictions.csv")
...
# ======================
def load_and_merge_data(conn, dataset_type: str):
  ...
  if dataset_type == 'train':
    ...
    WITH TrainBase AS (
      SELECT 
        ...
        COALESCE(AVG(physical_weight::FLOAT), 0) AS weight,
        COALESCE(AVG(physical_heartrate::FLOAT), 0) AS heartrate,
        COALESCE(MAX(sii), 0) AS sii -- Target column
      FROM TrainCsv
      GROUP BY id
      ),
      TrainActivity AS (
   ...
   # test_data = conn.execute(f"SELECT * FROM TestCsv;").df()
   # train_data = conn.execute(f"SELECT * FROM TrainCsv;").df()

   # Load Actigraphy data
   # actigraphy_test_data = conn.execute(f"SELECT * FROM ActigraphyTest;").df()
   # actigraphy_train_data = conn.execute(f"SELECT * FROM ActigraphyTrain;").df()

    # Merge datasets on 'id'
   # test = pd.merge(test_data, actigraphy_test_data, on = "id", how = "left")
   # train = pd.merge(train_data, actigraphy_train_data, on = "id", how = "left")
...
# ============================
def prepare_features_and_target(train, test, target_col = 'sii'):
...
def objective(trial, X_train, X_val, y_train, y_val):
    params = {
        "objective": "multiclass",
        "metric": "multi_logloss",
        "num_class": 4,  # Update based on number of classes
...
def run_optuna_tuning(X_train, y_train):
    ...
    # Define and optimize study
    study = optuna.create_study(direction="maximize")
    study.optimize(
      lambda trial: objective(
        trial, X_train_split, X_val, y_train_split, y_val), n_trials = 100)
...
def evaluate_and_save_model(
  model, X_test, y_test, output_model_path = "final_model.txt"):
    # Make predictions
...
def main_pipeline():
  ...
  #  actigraphy_test = "path/to/actigraphy_test.csv"
   # actigraphy_train = "path/to/actigraphy_train.csv"

    # Step 1: Load and Merge Data
    train, test = load_and_merge_data(conn)
    ...
    X_train, y_train, X_test, y_test = prepare_features_and_target(
      train, test, target_col='sii')
    ...
    evaluate_and_save_model(final_model, X_test, y_test)
...
def load_and_merge_data(
  ...
  dataset_type: str) -> pd.DataFrame:
    """
    Load, aggregate, and merge data for a specified dataset type ('train' or 'test').
    """
    if dataset_type == 'train':
        query = """
        WITH TrainBase AS (
...
def main_pipeline(
  ...
  target_col: str = 'sii'):
    ...
    # Train the final model
    final_model = train_final_model(conn, target_col, best_params)

    # Save the model
    final_model.save_model(f"{models_path}_model.txt")
    with open(f"{models_path}_params.json", 'w') as f:
...
main_pipeline(
  conn, 
  models_path = "data/output/lightgbm_optuna", 
  target_col = "sii")
...
r_series <- py$cv_summary

r_df <- data.frame(
  avg_train_logloss = r_series$avg_train_logloss,
  avg_val_logloss = r_series$avg_val_logloss,
  avg_train_accuracy = r_series$avg_train_accuracy,
  avg_val_accuracy = r_series$avg_val_accuracy
  )

"
- high training accuracy + lower validation accuracy: overfitting

- very poor performance on classification validation

- decent performance on classifcation training
"

notes_list = list(
  "Mean performance of the model on the training data.",
  "Mean performance of the model on the validation data.",
  "Correct classification on training data.",
  "Correct classification on validation data."
  )

locations_list = list("avg_train_logloss"
                      ,"avg_val_logloss"
                      ,"avg_train_accuracy"
                      ,"avg_val_accuracy"
                      )
...
pal_df <- tibble(
  cols = locations_list,
  pals = list(eval_palette("MetBrewer::Egypt", 4, 'd', 1))
  )
...
```{python}
ex_df = conn.sql(f"select * from ActigraphyTrain limit 10;").df()
ex_df
```
...
# Load exported data
train_data <- read_csv("data/output/train_data.csv")
train_data <- train_data |> as.data.frame()

test_results <- read_csv("data/output/test_results.csv")
test_results <- test_results |> as.data.frame()

cv_results <- fromJSON("data/output/cv_results.json")

cv_summary <- fromJSON("data/output/cv_summary.json")
...
# Example: Visualize age distribution in training data
r_plot <- train_data |>
  select(age) |>
  group_by(age) |>
  count(name = "count") |>
...
```{r}
#| label: "fig-valtrainAccuracy"

library(tidyr)

cv_df <- data.frame(
  fold = 1:length(cv_results$train_logloss),
  train_logloss = cv_results$train_logloss,
  val_logloss = cv_results$val_logloss,
  train_accuracy = cv_results$train_accuracy,
  val_accuracy = cv_results$val_accuracy
)

cv_long_accuracy <- cv_df |>
  pivot_longer(
    cols = c(train_accuracy, val_accuracy),
    names_to = "type",
    values_to = "accuracy"
  ) |>
  mutate(type = ifelse(type == "train_accuracy", "Train", "Validation"))

r_plot <- cv_long_accuracy |>
  ggplot(aes(x = factor(fold), y = accuracy, fill = type)) +
  geom_col(position = position_dodge(width = 0.75)) +
  coord_cartesian(ylim = c(0.50, 1.00)) +
  labs(
    title = "Accuracy Over Cross-Validation Folds",
    x = "Fold",
    y = "Accuracy"
  ) +
...
cv_long_logloss <- cv_df |>
  pivot_longer(
    cols = c(train_logloss, val_logloss),
    names_to = "type",
    values_to = "logLoss"
  ) |>
  mutate(type = ifelse(type == "train_logloss", "Train", "Validation"))

r_plot <- cv_long_logloss |>
  ggplot(aes(x = factor(fold), y = logLoss, fill = type)) +
  geom_col(position = position_dodge(width = 0.65)) +
  coord_cartesian(ylim = c(NA, 0.90)) +
  labs(
    title = "Log-Loss Over Cross-Validation Folds",
    x = "Fold",
    y = "Log-Loss"
  ) +
...
```{python}
#| label: "checkIdMatches"

checkSub = submission['id'] == test_results_renamed['id']
checkSub
```

```{r}
#| label: tbl-ijfoiejfoij
#| include: false
#| eval: false
...
```{r}
#| label: "tbl-submission"

r_df <- py$submission

notes_list =  list("Ids from test set.",
                   "Severity Impairment Index predicted value.")
