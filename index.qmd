...
```{python}
#| label: "mlModel"
#| code-summary: "An attempt at predictive modeling with ML."
...
# Define preprocessor builder function
...
# Functions for querying DuckDB tables
def execute_numeric_query(conn, table_name: str) -> pd.DataFrame:
  query = f"""
  SELECT id
         .COALESCE(AVG(CAST(basic_demos_age AS FLOAT)), 0) AS age
         .COALESCE(AVG(CAST(cgas_cgas_score AS FLOAT)), 0) AS cgas
         .COALESCE(AVG(CAST(physical_bmi AS FLOAT)), 0) AS bmi
         .COALESCE(AVG(CAST(physical_height AS FLOAT)), 0) AS height
         .COALESCE(AVG(CAST(physical_weight AS FLOAT)), 0) AS weight
         .COALESCE(AVG(CAST(physical_heartrate AS FLOAT)), 0) AS heartrate
  FROM {table_name}
  GROUP BY id;
  """
  return conn.execute(query).df()

def execute_ts_query(conn, table_name: str) -> pd.DataFrame:
    query = f"""
    SELECT id
           ,COALESCE(AVG(CAST(light AS FLOAT)), 0) AS light_mean
           ,COALESCE(STDDEV(CAST(light AS FLOAT)), 0) AS light_std
           ,COALESCE(MAX(CAST(light AS FLOAT)), 0) AS light_max
           ,COALESCE(AVG(CAST(hour_of_day AS FLOAT)), 0) AS hour_mean
           ,COALESCE(STDDEV(CAST(hour_of_day AS FLOAT)), 0) AS hour_std
...
def execute_cat_query(conn, table_name: str) -> pd.DataFrame:
    query = f"""
    SELECT DISTINCT id
...
# Merging features into a unified DataFrame
def load_and_merge_data(
  conn, dataset_type: str, target_col: str = None) -> pd.DataFrame:
    table_name = 'TrainCsv' if dataset_type == 'train' else 'TestCsv'
    ts_table_name = 'ActigraphyTrain' if dataset_type == 'train' else 'ActigraphyTest'
    
    numeric_df = execute_numeric_query(conn, table_name)
    ts_df = execute_ts_query(conn, ts_table_name)
    cat_df = execute_cat_query(conn, table_name)
    
    merged_df = (numeric_df
                 .merge(ts_df, on = 'id', how = 'left')
                 .merge(cat_df, on = 'id', how = 'left'))
    
    if dataset_type == 'train' and target_col:
        target_query = f"SELECT id, {target_col} FROM {table_name} GROUP BY id, {target_col};"
        targets = conn.execute(target_query).df()
        merged_df = merged_df.merge(targets, on = 'id', how = 'left')
...
def prepare_data(
  ...
  dataset_type: str, 
  target_col: str = None, 
  preprocessor = None) -> Tuple[np.ndarray, np.ndarray, ColumnTransformer]:
    
    df = load_and_merge_data(conn, dataset_type, target_col)
    
    numeric_cols = ['age', 'cgas', 'bmi', 'height', 'weight', 
    'heartrate', 'light_mean', 'light_std', 
    'light_max', 'hour_mean', 'hour_std']
    
    cat_cols = ['basic_demos_enroll_season', 'basic_demos_sex', 'common_weekday', 'common_quarter']
    
    # Drop rows with missing target values in training data
    if target_col and dataset_type == 'train':
        df = df.dropna(subset=[target_col])
    
    if preprocessor is None:
      preprocessor = build_preprocessor(numeric_cols, cat_cols)
      X = preprocessor.fit_transform(
        df.drop(columns = ['id', target_col], errors='ignore'))
    else:
        X = preprocessor.transform(
          df.drop(columns = ['id', target_col], errors='ignore'))
    
    y = df[target_col].values if target_col and dataset_type == 'train' else None
...
def cross_validate(
  ...
  n_splits: int = 5
    ...
    X, y, preprocessor = prepare_data(conn, 'train', target_col)
    ...
    for train_idx, val_idx in tss.split(X):
        ...
        cv_results['train_rmse'].append(
          np.sqrt(mean_squared_error(y_train, train_preds)))
        
        cv_results['val_rmse'].append(
          np.sqrt(mean_squared_error(y_val, val_preds)))
          
        cv_results['train_r2'].append(r2_score(y_train, train_preds))
        
        cv_results['val_r2'].append(r2_score(y_val, val_preds))
...
def train_final_model(
  conn, 
  target_col: str, 
  lgb_params: Dict[str, Any]) -> Tuple[lgb.Booster, ColumnTransformer]:
...
def evaluate_test_data(
  ...
  preprocessor: ColumnTransformer) -> pd.DataFrame:
    ...
    X_test, _, _ = prepare_data(
      conn, 'test', preprocessor = preprocessor)
...
def save_model(
  model: lgb.Booster, 
  preprocessor, lgb_params: Dict[str, Any], 
  path: str):
...
models_path = f"./data/models"

try:
  os.mkdir(models_path)
except FileExistsError:
  print(f"data/models exists")
```
...
# Cross-validation
cv_results, cv_summary = cross_validate(
  conn, target_col, lgb_params, n_splits=5)
print(cv_summary)

# Train final model
final_model, preprocessor = train_final_model(
  conn, target_col, lgb_params)

# Evaluate on test data
test_predictions = evaluate_test_data(
  conn, final_model, preprocessor)
print(test_predictions.head())

# Save model
save_model(
  final_model, preprocessor, lgb_params, "models_path")
...
#show(max_width=100)
with  open(
  "./data/profiles/actigraphy_train_profile.csv", 'w', encoding="utf-8") as outfile:
    with contextlib.redirect_stdout(outfile):
      x = conn.sql(f"SUMMARIZE SELECT * FROM ActigraphyTrain;").df()
      conn.execute(f"""
      COPY 
        {x} 
      TO  
...
```{python}
conn.execute(f"COPY {x} TO '/data/profiles/actigraphy_train_profile.csv' (HEADER , DELIMITER ',');")
```

```{python}
x = conn.sql(f"SUMMARIZE SELECT * FROM ActigraphyTrain;").df(max_width=100)
